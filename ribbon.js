...
class FilmRibbon {
  constructor(canvas, options = {}) {
    ...
    this.brand = options.brand || 'FRAME';
    this.disposed = false;
    this.gl = canvas.getContext('webgl', {
      alpha: false, antialias: true, powerPreference: 'low-power',
      preserveDrawingBuffer: options.preserveDrawingBuffer || false
    });
    this.ready = this.initialize(options.images || options.image || 'assets/portraits.webp');
  }
  compile(type, source) {
  ...
  roundedRect(ctx,x,y,width,height,radius){
    // Keep the atlas compatible with browsers predating Canvas roundRect.
  ...
  async initialize(imageUrl) {
    ...
    this.bg = this.program(`attribute vec2 aPosition; varying vec2 vUV; void main(){gl_Position=vec4(aPosition,0.,1.);vUV=vec2(aPosition.x*.5+.5,.5-aPosition.y*.5);}`, `precision mediump float;
      ...
      void main(){
        ...
        vec3 light=mix(vec3(.969,.949,.922),vec3(.894,.867,.827),vignette*.30);
        vec3 dark=mix(vec3(.104,.080,.061),vec3(.078,.063,.047),vignette*.85);
        float nearest=10.;
        for(int i=0;i<28;i++){
    ...
    this.film = this.program(`attribute vec4 aVertex; varying vec2 vUV; varying float vS; uniform vec4 uView; uniform float uEnter;
      ...
      void main(){vec4 pixel=texture2D(uAtlas,vec2(fract(vUV.x*.48-uOffset),vUV.y));if(pixel.a<.03)discard;
        ...
        vec3 color=pixel.rgb*(1.-.27*back);color=mix(color,vec3(.91,.85,.77),curl*.64);
    ...
    const loadImage = url => new Promise((resolve,reject)=>{
      ...
      image.onload=()=>resolve(image);image.onerror=()=>reject(new Error('Reel image could not load'));
      image.src=url;
    });
    const image = Array.isArray(imageUrl) ? await Promise.all(imageUrl.map(loadImage)) : await loadImage(imageUrl);
  ...
  createAtlas(image){
    ...
    for(let i=0;i<8;i++){
      const x=i*512;ctx.fillStyle='#28201b';ctx.fillRect(x,0,512,512);
      ctx.globalCompositeOperation='destination-out';
      for(let n=0;n<8;n++)for(const y of [17,442]){ctx.beginPath();this.roundedRect(ctx,x+12+n*64,y,38,49,7);ctx.fill();}
      ctx.globalCompositeOperation='source-over';ctx.save();ctx.beginPath();this.roundedRect(ctx,x+17,85,478,340,17);ctx.clip();
      if(Array.isArray(image)){
        const photo=image[i%image.length],ratio=478/340;
        const sw=Math.min(photo.width,photo.height*ratio),sh=sw/ratio;
        const sx=(photo.width-sw)/2,sy=(photo.height-sh)*.36;
        ctx.drawImage(photo,sx,sy,sw,sh,x+17,85,478,340);
      }else{
        // Compatibility with the original 4 × 2 contact-sheet source.
      ...
      ctx.restore();ctx.fillStyle='#deded3';ctx.textAlign='center';ctx.font='15px monospace';ctx.fillText(String(i+1),x+260,13);ctx.fillText(String(i+1),x+260,508);ctx.font='8px monospace';ctx.textAlign='left';ctx.fillText(this.brand,x+17,509);
    }
    return atlas;
  }
  resize(width,height,dpr=1){
    ...
    const scale=Math.min(dpr,limit/this.width,limit/this.height,Math.sqrt(2500000/(this.width*this.height)));
    this.canvas.width=Math.max(1,Math.floor(this.width*scale));this.canvas.height=Math.max(1,Math.floor(this.height*scale));
    this.gl.viewport(0,0,this.canvas.width,this.canvas.height);
  }
  draw(seconds, options={}){
    if(!this.initialized||this.disposed)return;
    const gl=this.gl,w=this.width||this.canvas.width,h=this.height||this.canvas.height;
    const mobile=w/h<.9,naturalWidth=mobile?Math.max(w,h*.84):w;
